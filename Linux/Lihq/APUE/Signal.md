...
- system()
  ...
  - **在有信号参与的程序当中，要阻塞住一个信号，要忽略调两个信号 这样 system()才能正常使用**

#### sleep 的缺陷

- **在某些平台，`sleep()`是使用`alarm`+`pause`封装的，而程序中出现多于 1 个的`alarm`alarm 将失效**
- 替代品
  - nanosleep()
  - usleep()
...
- sigsuspend() 信号驱动：解除对于某个信号集的阻塞操作之后马上进入等待信号阶段
...
- sigaction() 用来替换 signal() 还可以指定信号的来源以选择是否响应
