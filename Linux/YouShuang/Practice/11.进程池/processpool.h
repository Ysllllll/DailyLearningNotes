...
class process //描述一个子进程类，m_pid是目标子进程的PID，m_pipefd是父进程和子进程通信用的管道
{
public:
    process() : m_pid(0) {}
...
template <typename T> //进程池类，将它定义为模板类是为了代码复用，其模板参数是处理逻辑任务的类
class processpool
{
private:
        ...
        for (int i = 0; i < number; i++)
        {
            ...
            if ((sockfd == pipefd) && (events[i].events & EPOLLIN))
            {
                ...
                else
                {
                    ...
                    users[connfd].init(m_epollfd, connfd, client_address); //模板类T必须实现init方法
                }
            }
