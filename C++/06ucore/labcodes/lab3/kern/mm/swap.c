...
check_swap(void)
{
     ...
     pgfault_num = 0;
     // check_content_set() will generate multiple virtual addresses and access these addresses.
     // Since there is no physical memory space allocated to these virtual addresses at the initial time,
     // the do_pgfault function is called to initialize the corresponding entries in the page table
     // to complete the virtual real mapping
     check_content_set();
     assert(nr_free == 0);
     for (i = 0; i < MAX_SEQ_NO; i++)
          swap_out_seq_no[i] = swap_in_seq_no[i] = -1;

     // After the four free pages are allocated, nr_free = 0, There is no physical memory that can be allocated
     for (i = 0; i < CHECK_VALID_PHY_PAGE_NUM; i++)
     {
          check_ptep[i] = 0;
