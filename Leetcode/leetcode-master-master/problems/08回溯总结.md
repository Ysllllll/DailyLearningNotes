...
- 时间复杂度：O(n _ 2^n)，因为每一个元素的状态无外乎取与不取，所以时间复杂度为 O(2^n)，构造每一组子集都需要填进数组，又有需要 O(n)，最终时间复杂度：O(n _ 2^n)
...
>   - **[Leetcode 39. 组合总和](https://leetcode-cn.com/problems/combination-sum/submissions/)** [速览题解](#39)
